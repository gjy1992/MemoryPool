...
private:
	inline bool alloc_array()
	{
		...
		cur->next = (__helper_array*)malloc(BLOCKMEMSIZE);
		if (!cur->next)
			return false;
		capacity += blocksize;
		cur->next->last = cur;
	...
	inline bool dynamic_alloc_array()
	{
		...
		cur = cur->next;
		cur->count = 0;
		bkpchar *ptrpos = (bkpchar*)cur + sizeof(bkpulong) + sizeof(__helper*) * 3 + sizeof(__helper_array*) * 2;
		cur->curpos = (__helper*)ptrpos;
		cur->endpos = (__helper*)(ptrpos + blocksize * sizehelper);
		cur->freeList = NULL;
#ifdef _DEBUG
		for (bkpulong i = 0; i < blocksize; i++)
...
public:
	...
	{
		auto p = allocate();
		new ((void*)p) T(std::move(arg...));
		return p;
	}

	template <class ...args>
	T* op_new(const args&... arg)
	{
		auto p = allocate();
		new ((void*)p) T(arg...);
		return p;
